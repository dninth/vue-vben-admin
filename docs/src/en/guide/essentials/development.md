...
Take the addition of the test environment `test` to `@vben/web-ele` as an example:
...
"scripts": {
    ...
    "build:analyze": "turbo build:analyze",
    "build:ele": "pnpm run build --filter=@vben/web-ele",
    "build-test:ele": "pnpm run build --filter=@vben/web-ele build:test",
...
"tasks": {
    ...
    "build-test:ele": {
      "dependsOn": ["@vben/web-ele#build:test"],
      "outputs": ["dist/**"]
    },

    "@vben/web-ele#build:test": {
      "dependsOn": ["^build"],
      "outputs": ["dist/**"]
    },
